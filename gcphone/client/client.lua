  ...
  if GetPlayerServerId(PlayerId()) == tonumber(phoneId) then
    ESX.TriggerServerCallback('gcPhone:phone-check', function(durum)
      if durum ~= nil then
        ...
        ESX.ShowNotification("Telefonunuz Çalıyor!")
      end
    end)
  end
end)
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    ESX.TriggerServerCallback('gcPhone:phone-check', function(durum)
      if durum ~= nil then
        local text = 'Bir Yeni Mesaj'
        if ShowNumberNotification == true then
          if message.transmitter == "police" or message.transmitter == "ambulance" then
            text = message.transmitter.. ' İhbar Hattına 1 Yeni Mesaj'
          else
            text = message.transmitter.. ' Numaralı Kişiden Yeni Mesaj'
          end
          for _,contact in pairs(contacts) do
            if contact.number == message.transmitter then
              text = contact.display ..' Kişisinden Yeni Mesaj'
              break
            end
          end
        end
